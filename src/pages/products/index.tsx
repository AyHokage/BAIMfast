...
const MenuCategoryDetails = () => {
  ...
  return (
    ...
    <div className={`${styles.container} ${nunito.className}`}>
      <PositionRelative>
        <div className={styles.horizontal}><ProductFilters admin={false} setUrl={setUrl} /></div>
        {products.length > 0 ? 
        products.map((p: any, i: number) => <Product category='products' subcategory='sub' key={p.id} product={p} />) :
