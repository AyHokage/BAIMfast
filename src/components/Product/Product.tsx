    ...
    <div className={`${styles.horizontal} ${nunito.className}`}>                
      ...
      <div className={styles.productContent}>
        ...
        <p className={styles.desc}>
          { category !== 'services'  ? filterMarkdownElements(product.description) : product.description}
        </p>
        { category !== 'services'  ? <Link className={styles.link} href={`/${category}/${product.id}`}>Подробнее</Link> : <></>}
      </div> 
    </div>
  )
}
